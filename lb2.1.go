...
// Функция для итерации метода Зейделя
func gaussSeidelMethod(A [][]float64, b []float64, x []float64, iterations int, tolerance float64) ([]float64, int) {
	n := len(b)
	iterCount := 0
...
func main() {
	...
	// Количество итераций и допустимая погрешность
	iterations := 100
	tolerance := 1e-6

	// Вызываем метод Зейделя
	solution, iterCount := gaussSeidelMethod(A, b, x, iterations, tolerance)
	...
	fmt.Printf("Метод Зейделя завершил работу за %d итераций.\n", iterCount)
}
